...
| Language | Codebase | Simulated real task | Controlled credits | Cost USD | One debug/fix pass | Unconstrained agent range |
|---|---|---|---:|---:|---:|---:|
| C# | small | ASP.NET Core validation rule plus test | 21 | $0.21 | 24 | 82-144 |
| C# | medium | CQRS endpoint plus service plus integration tests | 69 | $0.69 | 84 | 310-551 |
| C# | large | Payment/status workflow across API, worker, persistence | 271 | $2.71 | 347 | 1,357-2,715 |
| Python | small | FastAPI validation fix plus unit test | 16 | $0.16 | 19 | 64-112 |
| Python | medium | Async ingestion job plus retry tests | 50 | $0.50 | 61 | 224-398 |
| Python | large | ETL pipeline refactor with backfill/retry behavior | 188 | $1.88 | 240 | 938-1,875 |
| React | small | Filterable table component | 20 | $0.20 | 23 | 78-137 |
| React | medium | Dashboard view using existing API state | 66 | $0.66 | 80 | 295-524 |
| React | large | Onboarding flow rework with routing/state/tests | 234 | $2.34 | 300 | 1,172-2,343 |
| JavaScript | small | Express middleware bugfix plus test | 18 | $0.18 | 21 | 71-123 |
| JavaScript | medium | Webhook handler plus queue handoff | 57 | $0.57 | 69 | 255-453 |
| JavaScript | large | Legacy module modernization across service boundary | 204 | $2.04 | 261 | 1,018-2,036 |
| TypeScript | small | Typed API client method plus tests | 19 | $0.19 | 23 | 77-135 |
| TypeScript | medium | NestJS module with DTO/service/tests | 62 | $0.62 | 76 | 279-497 |
| TypeScript | large | Shared type contract migration across frontend/backend | 224 | $2.24 | 287 | 1,120-2,241 |

## Reading The Numbers

Small tasks are usually cheap enough that the skill's main value is quality control, not raw cost savings.

Medium tasks are where the workflow starts paying off. A disciplined discovery-plan-scaffold-review loop typically stays under 100 credits, while an unconstrained implementation session can burn several hundred credits after repeated tool calls and revisions.

Large tasks are where the permission gate matters most. Cross-module work can stay in the low hundreds of credits when the agent plans and scaffolds instead of implementing everything. Letting the agent freely inspect, edit, run tests, and revise can move the same task into the 1,000-2,700 credit range.
...
- Small controlled tasks: usually under 1 percent of the monthly allowance.
- Medium controlled tasks: roughly 1-2 percent of the monthly allowance.
- Large controlled tasks: roughly 5-9 percent of the monthly allowance.
